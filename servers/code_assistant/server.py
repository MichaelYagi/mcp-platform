...
logger = setup_server_logging("mcp_code_assistant", PROJECT_ROOT, JsonFormatter)
...
def extend_code(file_path: str, description: str, write: bool = False) -> str:
    ...
    try:
        ...
        prompt = (
            ...
            f"- Do NOT reference external functions that do not exist in the file — implement everything inline, completely\n"
            f"- Do NOT use placeholder functions, hypothetical helpers, or stub calls — the generated code must be fully runnable as-is\n"
            f"- If the implementation requires helper functions, include them in the generated output\n"
            f"- Return ONLY the code block — no explanation, no breakdown, no commentary before or after the code\n"
            f"- Never hardcode absolute paths — use PROJECT_ROOT which is already defined in this file\n"
            f"- The function signature and parameter names must match exactly what is described — do not rename parameters\n"
            f"- Implementations that search or scan files must operate across the entire project, not just the file where the target is found\n"
