...
logger = setup_server_logging("mcp_code_review_server", PROJECT_ROOT, JsonFormatter)
...
@tool_meta(tags=["read","code","ai"],triggers=["review code","check code","audit code","code quality","code review","analyze code quality","check my code","look at my code"],idempotent=True,template='use review_code: path="" [max_bytes=""]',intent_category="code")
...
@mcp.tool()
@check_tool_enabled(category="code_reviewer")
@tool_meta(tags=["read","search","code"],triggers=["search code","find in code","grep code","search codebase","find function","find class","search for pattern","find in codebase","grep for"],idempotent=True,template='use search_code_in_directory: query="" [extension=""] [directory=""]',intent_category="code")
...
@mcp.tool()
@check_tool_enabled(category="code_reviewer")
@tool_meta(tags=["read","code","ai"],triggers=["debug","fix bug","fix error","analyze error","help debug","fix this error","what's wrong with this code","troubleshoot","diagnose error","why is this failing"],idempotent=True,template='use debug_fix: error_message="" [stack_trace=""] [code_snippet=""] [environment=""]',intent_category="code")
...
@mcp.tool()
@check_tool_enabled(category="code_reviewer")
@tool_meta(tags=["read"],triggers=["list capabilities","what can you do"],idempotent=True,template="use list_capabilities",intent_category="code")
