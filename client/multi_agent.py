...
CRITICAL: Never make up item IDs! Only use IDs returned by plex_find_unprocessed."""
    ...
    async def _create_execution_plan(self, user_request: str, skill_context: str = None) -> Optional[List[AgentTask]]:
        ...
        IMPORTANT: Return {{"subtasks": []}} for ANY of these cases:
        - Math, pattern recognition, or analytical questions answerable directly
        - Factual questions not requiring tool use
        - Requests that need only ONE agent with no dependencies
        - Anything a single LLM call can answer without tools

        Only use multiple agents when tasks are genuinely INDEPENDENT and PARALLEL.
