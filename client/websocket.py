...
async def process_query(websocket, prompt, original_prompt, agent_ref, conversation_state, run_agent_fn, logger, tools,
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════════════
        # COMMAND GUARD — never route colon-prefixed prompts to LLM
        # ═══════════════════════════════════════════════════════════════
        if prompt.lstrip().startswith(":"):
            # Commands are handled in the websocket loop before process_query.
            # If we end up here anyway, silently discard — don't send to LLM.
            logger.debug(f"[process_query] discarding unhandled command: {prompt[:40]}")
            return
    ...
    try:
        async for raw in websocket:
                    ...
                    for tool in tools:
                        ...
                        try:
                            ...
                            if schema is not None:
                                s = schema.model_json_schema() if callable(getattr(schema, 'model_json_schema', None)) else (schema.schema() if callable(getattr(schema, 'schema', None)) else {})
                                req = s.get("required", [])
                                props = s.get("properties", {})
                                # Handle Pydantic v2 $defs references — resolve $ref if needed
                                for p in req:
                                    if p not in props:
                                        continue
                                    prop = props[p]
                                    ...
                                    if "$ref" in prop:
                                        ...
                                        prop = s.get("$defs", {}).get(ref_key, prop)
                                    param_type = prop.get("type", "string")
                                    required_params.append({"name": p, "type": param_type})
            ...
            if data.get("type") == "user" or "text" in data:
                ...
                # Normalize: treat leading-space colon as command
                if prompt.lstrip().startswith(":"):
                ...
                if prompt.startswith(":"):
                    ...
                    if handled:
                        ...
                        # Unrecognized command — show error and valid commands
                        from client.commands import get_commands_list
                        _cmds = "\n".join(get_commands_list())
                        _err = f"❓ Unknown command: `{prompt}`\n\nAvailable commands:\n{_cmds}"
                        await broadcast_message("assistant_message", {"text": _err})
                        await websocket.send(json.dumps({"type": "complete", "stopped": False}))
                        continue
