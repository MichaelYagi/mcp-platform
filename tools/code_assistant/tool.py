...
def explain_code_impl(file_path: str, line_start: Optional[int], line_end: Optional[int], detail_level: str) -> str:
    """Explain code in a file using LLM."""
    import os
    ...
    detail_instructions = {
        "brief":    "Give a 2-3 sentence overview only.",
        "medium":   "Explain the purpose, key logic, and notable patterns in a few paragraphs.",
        "detailed": "Give a thorough explanation covering purpose, logic flow, design decisions, and any edge cases.",
    }
    ...
    prompt = (
        ...
        f"File: {p.name}\n"
        + (f"Lines {line_start}–{line_end}:\n" if line_start and line_end else "")
        + f"```\n{code[:6000]}\n```"
    )

    try:
        from langchain_ollama import ChatOllama
        MODEL_STATE_FILE = str(_Path(__file__).parent.parent.parent / "client" / "last_model.txt")
        model_name = open(MODEL_STATE_FILE).read().strip() if os.path.exists(MODEL_STATE_FILE) else "qwen2.5:latest"
        llm = ChatOllama(model=model_name, base_url=os.getenv("OLLAMA_BASE_URL", "http://127.0.0.1:11434"),
                         temperature=0.1)
        response = llm.invoke(prompt)
        return json.dumps({
            "file": file_path,
            "lines": f"{line_start}-{line_end}" if line_start and line_end else "all",
            "detail_level": detail_level,
            "explanation": response.content
        }, indent=2)
    except Exception as e:
        return json.dumps({"error": str(e), "file": file_path}, indent=2)


def generate_tests_impl(file_path: str, test_framework: str, coverage_target: str) -> str:
    """Generate unit tests for a code file using LLM."""
    import os
    from pathlib import Path as _Path
    ...
    framework_hint = "" if test_framework == "auto" else f" Use {test_framework} as the test framework."
    target_hint = {"functions": "Cover all public functions.", "classes": "Cover all public classes and methods.", "all": "Aim for comprehensive coverage."}.get(coverage_target, "")

    prompt = (
        f"Write unit tests for the following code.{framework_hint} {target_hint}\n"
...
def refactor_code_impl(file_path: str, refactor_type: str, target: str, preview: bool) -> str:
    ...
    type_instructions = {
        "extract_function": "Extract repeated or complex logic into a well-named function.",
        "rename": f"Rename {'`' + target + '`' if target else 'variables/functions'} to be more descriptive.",
        "simplify": "Simplify complex logic, reduce nesting, and improve readability.",
        "modernize": "Update to modern syntax: f-strings, type hints, walrus operator where appropriate.",
    }
    instruction = type_instructions.get(refactor_type, f"Apply '{refactor_type}' refactoring.")
    target_hint = f" Focus on: {target}." if target else ""

    prompt = (
        f"Refactor the following code. {instruction}{target_hint}\n"
        f"{'Show only the refactored code with a brief explanation of changes.' if preview else 'Return the complete refactored file.'}\n\n"
        f"File: {p.name}\n```\n{code}\n```"
    )
